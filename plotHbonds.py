...
# Read in number of frames from file (why? this info is in bonds.edges)
with open('frames.txt') as f:
	for line in f:
		frames = int(line.split('\n')[0])

# JRA: Added reading of bonds.info, removed reading of bonds.edges with dtype=None
# This is a new file I added to the output with more information to allow the y-axis tick labels to be changed
data = np.genfromtxt('bonds.info',dtype=None,encoding='utf-8')
#data = np.genfromtxt('bonds.edges',dtype=None)
# Read in occupancy data
data_occ = np.genfromtxt('bonds.info',usecols=0,encoding=None)
#data_occ = np.genfromtxt('bonds.edges',usecols=0)
# Read in list of donor atom numbers
data_don = np.genfromtxt('bonds.info',usecols=4,encoding=None)
#data_don = np.genfromtxt('bonds.edges',usecols=4)
# Read in list of acceptor atom numbers
data_acc = np.genfromtxt('bonds.info',usecols=8,encoding=None)
#data_acc = np.genfromtxt('bonds.edges',usecols=6)
...
if method == "Occ":
  ...
  if len(thresh) == 1:
    print("Only one value provided. Using it as the minimum occupancy and setting the maximum occupancy to 100%")
  ...
  # Each row of bonds.list contains a list of the frames in which the hbond referenced by the row number exists
  with open('bonds.list','r') as f:
    # cnt counts lines in f; c counts instances of the occupancy meeting the criteria
    cnt,c = 0,0
    # loop through the lines of bonds.list
    for line in f:
      ...
      if data_occ[cnt] >= t_min and data_occ[cnt] <= t_max:
        ...
        tick_.append(DonResName + ' ' + DonResNum + '-' + DonAtomName + ' : ' + AccResName + ' ' + AccResNum + '-' + AccAtomName)
        c += 1
      cnt += 1
...
# If 'number' is chosen
  ...
  # cnt counts lines in bonds.list, and we need to check each line in bonds.list for each item in val_at
    cnt = 0
    with open('bonds.list','r') as f:
      for line in f:
        line = list(map(int,map(float,line.rsplit())))
        ...
        if cnt == mem:
          ...
          c += 1
        # Increment cnt (essentially counts through lines in bonds.list)
        cnt += 1
    ...
    with open('bonds.list','r') as f:
      for line in f:
        ...
        if cnt == mem:
          ...
          c += 1
        cnt += 1
  # Place ticks at a spacing of 1, starting from 0.5, for as many ticks as there are entries in tick_, make the labels the entries in tick_, and set the font size
  pl.yticks(np.arange(0.5,len(tick_),1),tick_,fontsize=ytickFontsize)
  # JRA: Specify the xtick font size
  pl.xticks(fontsize=xtickFontsize)
  # JRA: Ashar set the x-axis limits to -100 and 100 more than the number of frames (why?!), and the y-axis limits to 0 and the number of entries in tick_
